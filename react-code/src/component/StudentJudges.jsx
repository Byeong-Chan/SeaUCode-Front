    ...
    for(let i = 0; i < props.problem_list.length; i++) {
        ...
        renders.push(
            <tr key={item.pending_number}>
                <td>
                    <Link to={"/studentJudges/"+item.pending_number}>
                        {item.pending_number}
                    </Link>
                </td>
                <td>
...
function StudentJudges(props) {
    const { url } = useRouteMatch();

    const { student_nickname, problem_number } = useParams();
    ...
    const changePage = e => {
        ...
        generalFunctions.loggedInTest(axios, cookies, dispatch)
            .then(result => {
                ...
                return axios.get(`/pending/getJudgeResultList/${nextPage}/${student_nickname}/${problem_number}`);
            })
            .then(result => {
    ...
    useEffect(() => {
        async function getFirstPage() {
            ...
            generalFunctions.loggedInTest(axios, cookies, dispatch)
                .then(result => {
                    generalFunctions.axiosInit(axios, result.refresh_token);
                    return axios.get(`/pending/getJudgeResultList/1/${student_nickname}/${problem_number}`);
                })
                .then(result => {
                    setSearchedJudgeList(result.data.judge_result_list);
                }).catch(err => {
    ...
    return (
        <div className="StudentJudges" style={{"height":"100%"}}>
            <Row>
                <Form.Label column sm="3"> {student_nickname} 학생의 제출 기록 </Form.Label>
                <Col sm="1">
...
export default withRouter(StudentJudges);