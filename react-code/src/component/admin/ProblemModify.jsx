...
function ProblemModify(props) {
    ...
    useEffect(() => {
        async function getJudgeResult() {
            ...
            axios.get(`/admin/getProblem/${id}`).then(res => {
                setCategory(res.data.Category);
                setTimeLimit(res.data.time_limit / 1000);
                setMemoryLimit(res.data.memory_limit / 1024 / 1024);
                setDifficulty(res.data.difficulty);
                setProblemDescription(res.data.problem_description);
                setOutputDescription(res.data.output_description);
                setInputDescription(res.data.input_description);
                setIsSpj(res.data.spj);
                ...
                for(let i = 0; i < res.data.io_length; i++) {
                    inputs.push({'_id': i, 'txt': ''});
                    outputs.push({'_id': i, 'txt': ''});
                }
                setInputList(inputs);
                setOutputList(outputs);
            }).catch(err => {
                if(err.response === undefined) alert('서버와의 연결이 끊어졌습니다.');
                else alert(err.response.message);
            });
        };
        getJudgeResult();
    }, [cookies, dispatch, id, props.history, removeCookies]);

    const modifyProblem = e => {
        ...
        if(form.spj_code === '') form.spj_code = undefined;
    ...
    return (
        <div className="ProblemModify" style={pageStyle}>
            <h3 style={{textAlign: "center"}}>문제 수정</h3>
            <hr/>
            ...
            <Row>
                ...
                <Col md="4" sm="12">
                    <Button variant="dark w-100" onClick={modifyProblem} style={completeBtnStyle}> 문제 수정! </Button>
                </Col>
            </Row>
        </div>
    );
...
export default withRouter(ProblemModify);